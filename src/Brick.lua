--[[
	...
	Author: Adam Gentry
	6/25/23
	-add black and lock bricks
	-change lock brick to black on hit 
		- only changes if unlock powerup is activated - can't break brick without it	
	-remove black brick on hit
...
paletteColors = {
    ...
    -- black - NEW - for black and lock bricks
    [6] = {
        ['r'] = 100,
        ['g'] = 100,
        ['b'] = 100
...
function Brick:hit()
    ...
    if self.tier > 0 then
        if self.color == 1 then
            ...
            self.color = 5
	elseif self.color == 6 then   -- NEW: if lock brick, change to black brick, but... 
		if unlocked == 1 then  -- ... only if key powerup activated
			self.tier = 0
		end
        else
        ...
        if self.color == 1 then
            self.inPlay = false
		elseif self.color == 6 then  -- NEW: 6, 0 - remove black brick
			self.inPlay = false    
        else
            self.color = self.color - 1
        end
